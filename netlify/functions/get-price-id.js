...
function getPriceIdKey(product, role, tier) {
  // Mini is sold online at list price for every buyer. Keep server-side Stripe
  // resolution pinned to the homeowner/MSRP Price ID even for logged-in pros.
  if (product === 'mini') {
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // and Bundle; Mini is flat MSRP for every role and can be bought online up to
    // Stripe's per-line-item max of 999,999.
