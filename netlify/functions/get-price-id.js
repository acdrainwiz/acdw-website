...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const isMiniProduct = product === 'mini'

    // Quantity ceiling.
    // Contractor / property-manager pricing tiers top out at 500 units for Sensor
    // and Bundle; Mini is sold online at flat MSRP for every buyer role.
    if (!isMiniProduct && userRole !== 'homeowner' && qty > 500) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({
    ...
    // Calculate tier
    let tier = 'msrp'
    if (!isMiniProduct && userRole !== 'homeowner') {
    ...
    // Get Price ID
    const pricingRole = isMiniProduct ? 'homeowner' : userRole
