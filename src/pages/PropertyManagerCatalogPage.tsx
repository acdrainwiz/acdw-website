...
export function PropertyManagerCatalogPage() {
  ...
  const pricingTable = getProductPricingTable(selectedProduct, selectedProduct === 'mini' ? 'homeowner' : 'property_manager')
  const currentTier = (selectedProduct === 'mini' ? 'msrp' : calculateTier(quantity)) as PricingTier
  const currentPrice = selectedProduct === 'mini' ? MSRP_PRICES.mini : getDisplayPrice(selectedProduct, 'property_manager', currentTier)
                ...
                <div key={product.id} className="hvac-pro-product-button-wrapper">
                  <button
                    onClick={() => {
                      ...
                      if (product.id !== 'mini' && quantity > 500) {
                        setQuantity(500)
                      }
