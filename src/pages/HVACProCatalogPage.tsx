...
export function HVACProCatalogPage() {
  ...
  const pricingTable = getProductPricingTable(selectedProduct, selectedProduct === 'mini' ? 'homeowner' : 'hvac_pro')
  const currentTier = (selectedProduct === 'mini' ? 'msrp' : calculateTier(quantity)) as PricingTier
  const currentPrice = selectedProduct === 'mini' ? MSRP_PRICES.mini : getDisplayPrice(selectedProduct, 'hvac_pro', currentTier)
